...
def active_session(user_id: int) -> WorkSession | None:
    with Session(engine) as s:
        return s.scalar(
            select(WorkSession).where(WorkSession.user_id == user_id, WorkSession.end_ts.is_(None)).order_by(WorkSession.id.desc())
        )

def add_log(user_id: int, kind: str, minutes: int | None = None, details: str | None = None):
    with Session(engine) as s:
        s.add(Log(user_id=user_id, kind=kind, minutes=minutes, ts=now_local().strftime("%Y-%m-%d %H:%M:%S"), details=details or ""))
        s.commit()
...
def month_totals(user_id: int):
    ...
    with Session(engine) as s:
        sessions = s.scalars(select(WorkSession).where(WorkSession.user_id == user_id, WorkSession.end_ts.is_not(None))).all()
...
with Session(engine) as s:
    logs = s.execute(select(Log.ts, Log.kind, Log.minutes, Log.details)
                     .where(Log.user_id == user["id"]).order_by(Log.id.desc()).limit(500)).all()
